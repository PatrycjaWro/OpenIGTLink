...
int ReceivePoint(igtl::Socket * socket, igtl::MessageHeader * header)
{
  ...
  if (c & igtl::MessageHeader::UNPACK_BODY) // if CRC check is OK
    ...
    for (int i = 0; i < nElements; i ++)
      ...
      igtlFloat32 minusValue[3];
      for (int j = 0; j < 3; j++) {
          minusValue[j] = pos[j] * -1;
      }

      igtl::PointElement::Pointer sendPoint;
      sendPoint = igtl::PointElement::New();
      sendPoint->SetName(pointElement->GetName());
      sendPoint->SetGroupName(pointElement->GetGroupName());
      sendPoint->SetRGBA(rgba);
      sendPoint->SetPosition(minusValue);
      sendPoint->SetRadius(pointElement->GetRadius());
      sendPoint->SetOwner(pointElement->GetOwner());


      pointMsg2->AddPointElement(sendPoint);
